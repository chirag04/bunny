...
public class InMemoryIntermediaryFilesRepository implements IntermediaryFilesRepository {
  ...
  public IntermediaryFileEntity get(UUID rootId, String filename) {
    Optional<IntermediaryFileEntity> intermediaryFileEntityOptional = get(rootId).stream().filter(intermediaryFileEntity -> intermediaryFileEntity.getFilename().equals(filename)).findFirst();
    return intermediaryFileEntityOptional.orElse(null);
  }
